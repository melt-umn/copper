...
public class CopperAntTask extends Task
{
	...
	throws BuildException
	{
		...
		if(!isDump) params.setDump(CopperDumpControl.OFF);
		else if(isDumpOnlyOnError) params.setDump(CopperDumpControl.ERROR_ONLY);
		else params.setDump(CopperDumpControl.ON);
		...
		if(skin != null) params.setUseSkin(skin);
		...
		files.add(Pair.cons(inputLabel,(Object) input));
		params.setInputs(files);
...
//	}
	...
	{
		return quietLevel.equals(CompilerLevel.QUIET);
	}
	...
	{
		return quietLevel.equals(CompilerLevel.VERBOSE);
	}
	...
	{
		return quietLevel.equals(CompilerLevel.VERY_VERBOSE);
	}
