...
public class ParserCompiler
{
	...
	private static String usageMessage(ParserCompilerParameters args)
	{
		...
		rv += "\t-cc\t\tUse ANSI color escape sequences in log output, even when logging to a file\n";
		rv += "\t-nc\t\tSuppress ANSI color in log output, even when logging to the terminal\n";
		...
		if(args != null && args.getPipeline() != null)
		{
			...
			if(customs != null && !customs.equals("") && !customs.equals("null"))
			{
