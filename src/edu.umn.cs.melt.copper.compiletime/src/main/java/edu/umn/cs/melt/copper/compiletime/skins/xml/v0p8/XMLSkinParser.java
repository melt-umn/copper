...
public class XMLSkinParser extends VersionSpecificXMLSkinParser {
	...
	throws ParseException,CopperException
	{
		...
		case PARSER_ELEMENT:
			if(currentExtendedParser != null) addParser(currentExtendedParser);
			else addParser(new ParserBean());
			...
			if(isUnitary != null && (isUnitary.equals("true") || isUnitary.equals("1"))) getCurrentParser().setUnitary(true);
			break;
	...
	public void characters(char[] ch, int start, int length)
	{
		//try
		//{
			charactersInner(ch,start,length);
		//}
		//catch(CopperException ex)
		//{
		//	error(new SAXParseException(ex.getMessage(),locator));			
	...
	throws CopperException
	{
		...
		case CHOICE_ELEMENT:
			...
			break;
		case CLASS_AUXILIARY_CODE_ELEMENT:
			getCurrentParser().setParserClassAuxCode(nodeText);
			break;
		case CLASS_ELEMENT:
			switch(peek().type)
			{
			...
			break;
		case CLASS_NAME_ELEMENT:
			getCurrentParser().setClassName(nodeText);
			break;
		case CODE_ELEMENT:
			switch(peek().type)
			{
		...
		case DECLARATIONS_ELEMENT:
			...
			break;
		case DEFAULT_PRODUCTION_CODE_ELEMENT:
			getCurrentParser().setDefaultProductionCode(nodeText);
			break;
		case DEFAULT_TERMINAL_CODE_ELEMENT:
			getCurrentParser().setDefaultTerminalCode(nodeText);
			break;
		case DISAMBIGUATE_TO_ELEMENT:
		...
		case INTERFACE_NAME_ELEMENT:
			...
			break;
		case INTERFACE_NAMES_ELEMENT:
			getCurrentParser().setInterfaceNames(interfaceNames);
			interfaceNames = null;
			break;
		case KLEENE_STAR_ELEMENT:
		...
		case OPERATOR_CLASS_REF_ELEMENT:
			...
			break;
		case PACKAGE_ELEMENT:
			getCurrentParser().setPackageDecl(nodeText);
			break;
		case PARSER_ATTRIBUTE_ELEMENT:
		...
		case PARSER_ELEMENT:
			...
			break;
		case PARSER_INIT_CODE_ELEMENT:
			getCurrentParser().setParserInitCode(nodeText);
			break;
		case POST_PARSE_CODE_ELEMENT:
			getCurrentParser().setPostParseCode(nodeText);
			break;
		case PP_ELEMENT:
			switch(peek().type)
			{
			...
			case PARSER_ATTRIBUTE_ELEMENT:
				...
				break;
			case PARSER_ELEMENT:
				getCurrentParser().setDisplayName(nodeText);
				break;
			case PRODUCTION_ELEMENT:
			...
			break;
		case PREAMBLE_ELEMENT:
			getCurrentParser().setPreambleCode(nodeText);
			break;
		case PRECEDENCE_ELEMENT:
			switch(peek().type)
			{
		...
		case RIGHT_ASSOCIATIVE_ELEMENT:
			...
			break;
		case SEMANTIC_ACTION_AUXILIARY_CODE_ELEMENT:
			getCurrentParser().setSemanticActionAuxCode(nodeText);
			break;
		case SINGLE_CHARACTER_ELEMENT:
			// Empty
			break;
		case START_LAYOUT_ELEMENT:
			getCurrentParser().setStartLayout(refSet);
			refSet = null;
			break;
		case START_SYMBOL_ELEMENT:
			getCurrentParser().setStartSymbol(refList.get(0));
			refList = null;
			break;
		case SUBMITS_ELEMENT:
