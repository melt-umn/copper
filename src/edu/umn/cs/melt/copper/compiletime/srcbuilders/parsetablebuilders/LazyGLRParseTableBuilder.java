...
import edu.umn.cs.melt.copper.compiletime.parsetable.old.LazyGLRParseTable;
...
public class LazyGLRParseTableBuilder extends ParseTableBuilder implements ParseActionVisitor<String,CopperException>
{
	...
	public LazyGLRParseTableBuilder(GrammarSource grammar,
			                    LALR1DFA dfa,
	...
	throws CopperException
	{
		...
		out.print("    public static ThisParseTable parseTable;\n\n");

		out.print("    private static class ThisParseTable extends " + LazyGLRParseTable.class.getName() + "\n");
		out.print("    {\n");
		out.print("        public ThisParseTable()\n");
		out.print("        {\n");
		out.print("            super(" + (sortedStates.last() + 1) + ");\n");
		out.print("        }\n");
		out.print("        public void initShiftableUnion()\n");
		out.print("        {\n");
		out.print("            shiftableUnion = tset(");
		for(Iterator<Terminal> it = parseTable.getShiftableUnion().iterator();it.hasNext();)
		{
			Terminal t = it.next();
		...
		out.print(");\n");
		out.print("        }\n");
		out.print("        public void initState(int statenum)\n");
		out.print("        {\n");
		out.print("            switch(statenum)\n");
		out.print("            {\n");
		for(int statenum : sortedStates) out.print("                case " + statenum + ": init_" + statenum + "(); return;\n");
			...
			if(parseTable.hasShiftable(statenum))
			{
				for(Terminal t : parseTable.getShiftable(statenum))
				{
					...
					for(ParseAction action : actions)
					{
						if(j > 0)
						{
							if(logger.isLoggable(CompilerLogMessageSort.ERROR)) logger.logMessage(CompilerLogMessageSort.ERROR,null, "Parse table conflict between/among actions " + actions + " in cell (" + statenum + "," + t + ")");
						}
	...
	throws CopperException
	{
		...
		out.print("        parseTable = new ThisParseTable();\n");
	}
